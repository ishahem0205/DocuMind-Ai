...
interface ChatInterfaceProps {
  file: UploadedFile;
  messages: ChatMessage[]; // controlled by parent (App)
  onMessagesChange: (msgs: ChatMessage[]) => void;
}

export const ChatInterface: React.FC<ChatInterfaceProps> = ({ file, messages, onMessagesChange }) => {
  const [input, setInput] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const chatSessionRef = useRef<any>(null);
  const messagesEndRef = useRef<HTMLDivElement>(null);

  // Initialize chat session on mount or when file changes
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-[600px] bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
        {/* Chat Header */}
        <div className="bg-slate-900/50 p-4 border-b border-slate-700 flex items-center justify-between gap-3">
             ...
             <div className="flex items-center gap-2">
               <button
                 onClick={() => {
                   if (!confirm('Clear saved chat session for this file?')) return;
                   const key = `docuChat:${file.file.name}`;
